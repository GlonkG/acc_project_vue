    ...
    <div id="order_num" v-for="items in order">
      ...
      <div class="order_body">
        ...
        <div class="zhuangtai">
          ...
          <div class="goback">
          <el-link type="primary" style="font-size: 18px;" @click="goback(orderNo)">去支付</el-link>
          </div>
        </div>
      </div>
...
<script>
  ...
  export default{
    ...
    methods:{
      ...
      query:function () {
        var oStatus = 0;
        var url = 'api/accplayer-order/findByoStatus/'+ oStatus;
      ...
      goback:function (orderNo) {
        ...
        axios.get(url).then(res=> {
          if (res.data != null) {
            ...
            this.order = res.data.list;
            for (var i = 0; i < this.order.length; i++) {
              this.order.orderNo = this.order[i].orderNo;
              //alert(this.order[0].orderNo);
            }
